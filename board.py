...
class Apppage(tk.Frame):
	...
	# Def Search
	def Create_Label(self,rows,columns,info,place,bds = None):
		...
		for row in range(rows):
			...
			for column in range(columns):
				if column == 0:
					Entry = tk.Label(place,text =info[row][column],font=('Arial',22),bg='white',image=self.virtual_image,compound='c',bd=bds)
					aRow.append(Entry)
					sticky='e'
				else:
					var_entry = tk.StringVar()
					Entry = tk.Entry(place,font=('Arial',22),bg='white',width=self.width_label,textvariable = var_entry,bd=bds)
	...
	def Create_footer_Label(self,place,arg):
		for feature in self.features:
			index =  self.features.index(feature)
			button = tk.Button(place,text = feature,bg='white',bd=0,fg='black',font=('Arial',12,'bold'),command=self.command[index])
	...
	def Plus_Number(self,number,info):
		...
		self.table_info[info][number].delete(0,'end')
		try:
			self.table_info[info][number].insert('end',(int(Number)+1))
		except ValueError:
			self.table_info[info][number].insert('end',('Format angka'))
	...
	def Delete_Size(self):
		...
		self.settings.Write_Data(self.settings.data_place,self.settings.data)
		self.refresh_frame()
	...
	def Find_Types(self):
		Find_Types = self.var_name.get().lower()
		if Find_Types != '':
			self.List_1_box.delete(0,'end')

			for all_index in self.settings.data:
				for all_types in all_index:
					if Find_Types.lower() in all_types.lower() :
	...
	def Find_Brand(self):
		Find_Types = self.var_name_2.get().lower()
		if Find_Types != '':
			self.List_2_box.delete(0,'end')
			...
			for all_index,info in self.settings.data[self.index].items():
				for Brand,Brand_info in info.items():
					for Brand_Name, Brand_infos in Brand_info.items():
						if Find_Types in Brand_Name.lower() :
	...
	def add_Debugging(self):
		try:
			...
			if Error_enter <= 1:					
				self.table_info[0][1].delete(0,'end')
				self.table_info[0][1].insert('end','Format Angka')

				self.table_info[1][1].delete(0,'end')
				self.table_info[1][1].insert('end','Format Angka')
	...
	def Add_Data(self):
		...
		if self.Datas != None:
			confirm = msg.askyesnocancel('Add Data Save Confirmation','Are you sure to add this ?')
			if confirm:
				...
				if self.Brand_list != None :
					...
					if self.ComboDatas[0] in self.product_type:
						New_datas = {self.ComboDatas[1]:{str(self.Datas[2][0])+satuan: {"Stock" : str(self.Datas[0][0]),"Harga" : str(self.Datas[1][0])}}}
						self.settings.data[self.index][self.ComboDatas[0]]['Brand'].update(New_datas)
					else:
	...
	def Update_Data(self):	
		...
		if self.Datas != None:
			...
			if confirm:
				...
				self.settings.Write_Data(self.settings.data_place,self.settings.data)
				self.right_update_frame.destroy()
				self.refresh_frame()
			elif confirm == False:
				self.right_update_frame.destroy()
				self.refresh_frame()
	...
	def Forget_bind(self):
		self.List_1_box.unbind('<<ListboxSelect>>')
		self.List_2_box.unbind('<<ListboxSelect>>')
	...
	def Delete_Brand(self):
		...
		self.refresh_frame()
		self.settings.Write_Data(self.settings.data_place)
	...
	def create_Update_content(self):
		...
		self.Create_Label(rows,columns,info,self.detail_update_content,0)
	...
	def create_Update_footer(self):
		...
		self.features = ['Save','Cancel']
		self.button_update_features = []
		self.command = [self.Update_Data,self.refresh_frame]
		self.Create_footer_Label(self.detail_update_footer,self.button_update_features)
	...
	def create_Add_header(self):
		...
		self.Volumetypes_add_Combo['values'] = self.settings.satuan
	...
	def create_Add_content(self):
		...
		self.Create_Label(rows,columns,info,self.detail_add_content)
	...
	def create_Add_footer(self):
		...
		self.features = ['Save','Cancel']
		self.command = [self.Add_Data,self.refresh_frame]
		self.button_add_features = []
		self.Create_footer_Label(self.detail_add_footer,self.button_add_features)
